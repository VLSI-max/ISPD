...
void globalPlace() {
	...
	constructQuadraticProblem(); //测试通过 constructQuadraticProblem()
								 //测试通过 edge connection；g_place_qpProb->connect[i]
								 //测试通过 edge weight；g_place_qpProb->edge_weight[i]
	...
	while (!completionFlag) {
		...
		solveQuadraticProblem(!IGNORE_COG);  //测试通过
		...
		//if (iteration == 3) break; //wu: 调试
		iteration++;
		//cout << "iteration = " << iteration << endl; //wu edit 调试
	}
