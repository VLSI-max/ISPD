...
void constructQuadraticProblem() {
	...
	ConcreteCell  *cell;
	ConcreteNet   *net;
	int           *cell_numTerms = (int*)calloc(g_place_numCells, sizeof(int));
	ConcreteNet ***cell_terms = (ConcreteNet ***)calloc(g_place_numCells, sizeof(ConcreteNet**));
	...
	g_place_qpProb->area = (qps_float_t*)realloc(g_place_qpProb->area,
		sizeof(float)*g_place_numCells);// "area" matrix
	g_place_qpProb->edge_weight = (qps_float_t*)realloc(g_place_qpProb->edge_weight,
		sizeof(float)*maxConnections);  // "weight" matrix
	g_place_qpProb->connect = (int*)realloc(g_place_qpProb->connect,
		sizeof(int)*maxConnections);    // "connectivity" matrix
	g_place_qpProb->fixed = (int*)realloc(g_place_qpProb->fixed,
...
int generateCoGConstraints(reverseCOG COG_rev[]) {
	...
	int cogRevNum = 0;
	Partition **stack = (Partition **)malloc(sizeof(Partition*)*g_place_numPartitions * 2);
	int stackPtr = 0;
	Partition *p;
	float cgx, cgy;
...
void solveQuadraticProblem(bool useCOG) {
	...
	reverseCOG *COG_rev = (reverseCOG *)malloc(sizeof(reverseCOG)*g_place_numPartitions);

	g_place_qpProb->cog_list = (int*)malloc(sizeof(int)*(g_place_numPartitions + g_place_numCells));
	g_place_qpProb->cog_x = (qps_float_t*)malloc(sizeof(float)*g_place_numPartitions);
	g_place_qpProb->cog_y = (qps_float_t*)malloc(sizeof(float)*g_place_numPartitions);
